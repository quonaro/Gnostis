...
func TestReindexFiles_OutsideProject(t *testing.T) {
	...
	res, err := srv.reindexFiles(context.Background(), mcp.CallToolRequest{}, reindexFilesArgs{Paths: []string{"/etc/passwd"}})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !res.IsError {
...
func TestReindexFiles_Directory(t *testing.T) {
	dir := t.TempDir()
	srv := New("test", "1.0.0", &mockSearcher{}, nil, &mockReindexer{}, []project.Project{{Name: "test", Path: dir}})
	res, err := srv.reindexFiles(context.Background(), mcp.CallToolRequest{}, reindexFilesArgs{Paths: []string{dir}})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !res.IsError {
		t.Fatal("expected error result for directory path")
	}
}
